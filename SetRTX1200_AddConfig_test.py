...
config.read('./SetRTX1200_AddConfig_parameter')

ip = config.get('General', 'ip_lan3')
...
command.send('dhcp scope option ' + config.get('DHCP', 'scope_num') + config.get('DHCP', 'dhcp_primary_dns') + ',' + config.get('DHCP', 'dhcp_secondary_dns') + '\n')
#command.send("dns server 202.224.32.1 202.224.32.2\n")
command.send('dns server ' + config.get('General', 'net_primary_dns') + config.get('DHCP', 'net_secondary_dns') + '\n')
...
command.send('no ip lan3 address ' + config.get('General', 'ip_lan3') + '\n')
command.send("save\n")
time.sleep(2.5)
